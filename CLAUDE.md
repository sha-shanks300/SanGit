...
- **All audio playback uses short-lived signed URLs** minted server-side from private buckets (`flp-files`, `audio`) — never public storage URLs. This is what makes private share links (`share_links` table: expiring, revocable, view-logged) enforceable. Private-link access bypasses RLS via `GET /api/listen/[token]` (`lib/share-token.ts` resolves tokens). Links are **project-scoped by default** (`version_id null` → read-only project view at `/s/[token]`: all versions, Tree/Graph, player, comments) or version-scoped (single-track player). Signed-in visitors like/comment on private shared projects through token-authenticated routes (`/api/listen/[token]/{audio,interactions,reactions,comments}`) that validate token + session and write via the admin client — RLS can't see the token, so don't loosen it. Only the payload GET counts a view; sub-resource fetches re-check revocation/expiry but never burn views. Images (avatars, banners, project artwork) are the exception to signed URLs: they live in the **public `public-images` bucket**, uploaded directly from the browser (`lib/image-upload.ts`, storage RLS scopes writes to `{user_id}/`), timestamped filenames, never overwritten.
...
2. **Graph** — Obsidian-style interactive physics view in `components/version-graph.tsx` using **`react-force-graph-2d`** with `dagMode="lr"`. Chosen over React Flow + dagre because `dagMode` re-applies the DAG constraint every simulation tick, so dragged nodes are continuously pulled back and a descendant can never settle upstream of its ancestor — no custom physics math. Constraint is enforced by **topological depth, not timestamp** (cross-branch wall-clock order is not guaranteed; within any ancestry chain it is). If global time ordering is ever needed, replace `dagMode` with a `d3.forceX` targeting `uploaded_at`.
   ...
   - Canvas nodes are painted via `nodeCanvasObject` with colors read from the CSS variables at runtime (canvas can't use Tailwind); tooltip is a Tailwind-styled absolutely-positioned div driven by `onNodeHover`. Split into `version-graph.tsx` (thin `next/dynamic` + `ssr:false` wrapper) and `version-graph-canvas.tsx` (static import, holds the `ForceGraphMethods` ref needed for custom d3 forces).
   - Drag semantics: elastic play, not placement — `onNodeDrag` reheats the simulation every tick so neighbours tug along, and `onNodeDragEnd` clears `fx`/`fy` so the dag + zig-zag forces spring the graph back to its rest order. Nothing persists; releasing (or refreshing) restores the original layout. Pointer hit area is NODE_R+9 so the small dots are grabbable.
   - Initial layout is zig-zagged: `graph-data.ts` computes a `zigY` target per node (branch lane ± alternating stagger) and seeds `y`; a weak custom `d3Force("zigzag")` in the canvas component pulls unpinned nodes toward it.
...
`apps/web/DESIGN.md` is the authoritative visual spec (Ferrari-derived cinematic editorial dark theme). Non-negotiables: canvas `#181818` (never pure black), brightness-step surface ladder + 1px hairline borders instead of shadows, a single Rosso Corsa accent `#da291c` used scarcely (primary CTA, wordmark, Main-version indicator, playhead), success green `#03904a` as the only other chromatic color (focus rings are yellow `#f6e500`), **sharp 0px corners on every button/card/panel** (pill geometry only for status badges/avatars), display type at weight 400 — never bold, three-tier fonts: Space Grotesk (display) + Inter (body) + JetBrains Mono (nav links, eyebrows, badges, hashes). No light mode, no second accent; the only gradient is the brand-red CTA hover.
