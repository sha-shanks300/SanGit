 ...
 * cover and fades in a Rosso play button; the active track keeps it visible as
 * a pause. Rendered only where a player queue exists (the dashboard grid) —
 * elsewhere ProjectRow shows the plain artwork, so this hook never runs
 * outside a <PlayerProvider>.
 *
  ...
  return (
    <div className={cn("group relative", className)}>
      ...
      <button
        ...
        className={cn(
          "absolute inset-0 flex items-center justify-center bg-canvas/50 transition-opacity",
          isActive
            ? "opacity-100"
            : "opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none"
        )}
      >
        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-white transition-transform group-hover:scale-105">
          {isPlaying ? (
